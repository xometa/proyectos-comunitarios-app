...
public class listaCSP {
    ...
    public void insertar(Preliquidaciones dato) {
        NodoCS nuevo = new NodoCS(dato);
        if (isEmpty()) {
            nuevo.setSiguiente(nuevo);
            lista = nuevo;
        } else if (dato.getClasedeobra().compareToIgnoreCase(((Preliquidaciones)lista.getDato()).getClasedeobra()) < 0) {
            ...
            NodoCS aux = ubicar(dato.getClasedeobra());
            nuevo.setSiguiente(aux.getSiguiente());
            aux.setSiguiente(nuevo);
        }
    }
    
     public void insertaralinicio(Preliquidaciones dato) {/*metodo que permite ingresar un nodo al inicio de la lista*/
        NodoCS nuevo = new NodoCS(dato);
        if (isEmpty()) {
            nuevo.setSiguiente(nuevo);
            lista = nuevo;
    ...
    public void insertaralfinal(Preliquidaciones dato) {/*metodo para insertar un nodo al final de los que ya existen o posicionarlo
    ...
    public void insertarantesde(Preliquidaciones dato, String antes) {/*metodo para insertar un nodo antes de un parametro que se 
        recibe, con el fin de agregar el nuevo registro antes de cualquier nodo ya registrado*/
        NodoCS nuevo = new NodoCS(dato);
        if (isEmpty()) {
            nuevo.setSiguiente(nuevo);
            lista = nuevo;
        } else if (antes.compareToIgnoreCase(((Preliquidaciones)lista.getDato()).getClasedeobra()) == 0) {
            NodoCS aux = lista;
            while (aux.getSiguiente() != lista) {/*while para posicionar el nodo, se ejecutara mientras sea diferente del 
                inicio para poder posicionar el nodo*/
                aux = aux.getSiguiente();
    ...
    public void insertardespuesde(Preliquidaciones dato, String despues) {/*metodo para insertar un nodo despues de cualquier nodo 
        que ya existe, que se manda como parametro y se comprueba que exista en la lista*/
        NodoCS nuevo = new NodoCS(dato);
        if (isEmpty()) {
            nuevo.setSiguiente(nuevo);
            lista = nuevo;
        } else if (despues.compareToIgnoreCase(((Preliquidaciones)lista.getDato()).getClasedeobra()) == 0) {
            NodoCS aux = lista;
            do {
                aux = aux.getSiguiente();
    ...
    public NodoCS ubicar(String dato) {
        NodoCS aux = lista;
        NodoCS ant = lista;
        while (aux.getSiguiente() != lista && dato.compareToIgnoreCase(((Preliquidaciones)aux.getDato()).getClasedeobra()) > 0) {
            ant = aux;
            aux = aux.getSiguiente();
        }
        if (dato.compareToIgnoreCase(((Preliquidaciones)aux.getDato()).getClasedeobra()) > 0) {
            ant = aux;
        }
        return ant;
    }
    ...
    public ArrayList<Preliquidaciones> mostrarDatos() {
